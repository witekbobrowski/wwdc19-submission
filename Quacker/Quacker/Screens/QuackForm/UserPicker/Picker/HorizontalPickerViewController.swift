...
class HorizontalPickerViewController: UIViewController {
    ...
    var current: Int? { didSet { scrollToItem(current) } }
    ...
    @objc private func didTapItem(sender: UIGestureRecognizer) {
        guard let view = sender.view else { return }
        current = stackView.arrangedSubviews.firstIndex(where: { $0 == view })
    }
...
extension HorizontalPickerViewController {
    ...
    private func updateStackView(with content: [UIViewController]) {
        ...
        content.compactMap { $0.view }.forEach { view in
    ...
    private func scrollToItem(_ index: Int?) {
        guard let index = index, index < contentViewControllers.count  else { return }
...
extension HorizontalPickerViewController: UIScrollViewDelegate {
